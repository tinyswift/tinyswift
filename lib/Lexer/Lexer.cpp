...
Lexer::Lexer(const LangOptions &Options, const SourceManager &SM,
             DiagnosticEngine *Diags, unsigned BufferID, bool InSILMode,
             CommentRetentionMode RetainComments)
    : LangOpts(Options), SourceMgr(SM), Diags(Diags), BufferID(BufferID),
      InSILMode(InSILMode), RetainComments(RetainComments) {
  // Initialize buffer pointers.
  llvm::StringRef contents = SM.extractText(SM.getRangeForBuffer(BufferID));
...
void Lexer::initSubLexer(Lexer &Parent, State BeginState, State EndState) {
  ...
  if (Parent.ArtificialEOF && Parent.ArtificialEOF >= BeginStatePtr &&
...
Token Lexer::getTokenAt(SourceLoc Loc) {
  assert(BufferID ==
             static_cast<unsigned>(SourceMgr.findBufferContainingLoc(Loc)) &&
...
void Lexer::formToken(tok Kind, const char *TokStart) {
  ...
  NextToken.setToken(Kind, llvm::StringRef(TokStart, CurPtr - TokStart),
                     CommentLength);
}
...
void Lexer::lexIdentifier() {
  ...
  tok Kind =
      kindOfIdentifier(llvm::StringRef(TokStart, CurPtr - TokStart), InSILMode);
...
void Lexer::lexOperatorIdentifier() {
  ...
  do {
    ...
    if (Identifier::isEditorPlaceholder(
            llvm::StringRef(CurPtr, BufferEnd - CurPtr)) &&
...
void Lexer::lexHexNumber() {
  ...
  if (!llvm::isDigit(*CurPtr)) {
    if (PtrOnDot && !llvm::isDigit(PtrOnDot[1]) && !signedExponent) {
...
void Lexer::lexNumber() {
  const char *TokStart = CurPtr - 1;
  assert((llvm::isDigit(*TokStart) || *TokStart == '.') && "Unexpected start");
  ...
  if (*CurPtr == '.') {
    ...
    if (!llvm::isDigit(CurPtr[1]) || NextToken.is(tok::period))
      return formToken(tok::integer_literal, TokStart);
  } else {
...
llvm::StringRef
Lexer::getEncodedStringSegment(llvm::StringRef Bytes,
...
void Lexer::getStringLiteralSegments(
    const Token &Str, llvm::SmallVectorImpl<StringSegment> &Segments,
    DiagnosticEngine *Diags) {
  assert(Str.is(tok::string_literal));
  // Get the bytes behind the string literal, dropping the double quotes.
  llvm::StringRef Bytes = Str.getText().drop_front().drop_back();
  ...
  while (BytesPtr != Bytes.end()) {
    ...
    Segments.push_back(StringSegment::getLiteral(
        getSourceLoc(SegmentStartPtr), BytesPtr - SegmentStartPtr - 2));

    // Find the closing ')'.
    const char *End =
        skipToEndOfInterpolatedExpression(BytesPtr, Str.getText().end(), Diags);
  ...
  Segments.push_back(StringSegment::getLiteral(getSourceLoc(SegmentStartPtr),
...
SourceLoc Lexer::getLocForStartOfToken(SourceManager &SM, SourceLoc Loc) {
  llvm::Optional<unsigned> BufferIdOp =
      SM.getIDForBufferIdentifier(SM.getBufferIdentifierForLoc(Loc));
  if (!BufferIdOp.hasValue())
    return SourceLoc();
  return getLocForStartOfToken(
      SM, BufferIdOp.getValue(),
...
llvm::StringRef Lexer::getIndentationForLine(SourceManager &SM, SourceLoc Loc) {
  ...
  while (*EndOfIndentation && clang::isHorizontalWhitespace(*EndOfIndentation))
    ++EndOfIndentation;
