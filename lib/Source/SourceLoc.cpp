...
SourceManager::addNewSourceBuffer(std::unique_ptr<llvm::MemoryBuffer> Buffer) {
  assert(Buffer);
  llvm::StringRef BufIdentifier = Buffer->getBufferIdentifier();
...
unsigned SourceManager::addMemBufferCopy(llvm::StringRef InputData,
...
bool SourceManager::openVirtualFile(SourceLoc loc, llvm::StringRef name,
                                    int lineOffset) {
...
SourceManager::getIDForBufferIdentifier(llvm::StringRef BufIdentifier) {
...
llvm::StringRef
SourceManager::extractText(CharSourceRange Range,
...
void SourceLoc::print(llvm::raw_ostream &OS, const SourceManager &SM,
...
void SourceLoc::dump(const SourceManager &SM) const { print(llvm::errs(), SM); }

void SourceRange::print(llvm::raw_ostream &OS, const SourceManager &SM,
...
void CharSourceRange::print(llvm::raw_ostream &OS, const SourceManager &SM,
                            unsigned &LastBufferID, bool PrintText) const {
  OS << '[';
  ...
  if (PrintText) {
    OS << " RangeText=\""
       << llvm::StringRef(Start.Value.getPointer(),
                          getEnd().Value.getPointer() -
                              Start.Value.getPointer() + 1)
       << '"';
  }
}
