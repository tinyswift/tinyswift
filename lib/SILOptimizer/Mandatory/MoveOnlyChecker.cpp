...
class MoveOnlyCheckerPass : public SILFunctionTransform {
    ...
    for (auto &bb : *fn) {
      for (auto *arg : bb.getArguments()) {
        ...
        for (auto *use : arg->getUses()) {
          if (isa<MarkUnresolvedNonCopyableValueInst>(use->getUser())) {
            alreadyMarked = true;
            break;
          }
        }
        ...
        builder.setInsertionPoint(bb.front().getIterator());
        builder.createMarkUnresolvedNonCopyableValueInst(
  ...
  void run() override {
    ...
    // TinySwift: inject move-only markers for non-trivial types with deinit.
    bool injectedMarkers = injectTinySwiftMoveOnlyMarkers(fn);
    ...
    checker.madeChange |= injectedMarkers;
