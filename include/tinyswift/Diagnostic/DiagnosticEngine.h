...
public:
  ...
  /*implicit*/ Diagnostic(DiagID ID, llvm::ArrayRef<DiagnosticArgument> Args)
  ...
  llvm::ArrayRef<CharSourceRange> getRanges() const { return Ranges; }
...
public:
  ...
  /// diagnostic.
  InFlightDiagnostic &fixItReplace(SourceRange R, llvm::StringRef Str);
  ...
  InFlightDiagnostic &fixItInsert(SourceLoc L, llvm::StringRef Str) {
  ...
  ///
  InFlightDiagnostic &fixItInsertAfter(SourceLoc L, llvm::StringRef);
...
public:
  ...
  //                 typename detail::PassArgument<ArgTypes>::type... Args) {
  //            assert(!ActiveDiagnostic && "Already have an active
  //            diagnostic"); ActiveDiagnostic = Diagnostic(ID,
  //            std::move(Args)...);
  ...
  //                                    {
  //            assert(!ActiveDiagnostic && "Already have an active
  //            diagnostic"); ActiveDiagnostic = Diagnostic(id, args);
  ...
  /// \returns An in-flight diagnostic, to which additional information can
  /// be attached.
  //        InFlightDiagnostic diagnose(const Decl *decl, const Diagnostic
  //        &diag) {
  //            assert(!ActiveDiagnostic && "Already have an active
  //            diagnostic"); ActiveDiagnostic = diag;
  //            ActiveDiagnostic->setDecl(decl);
  //            return InFlightDiagnostic(*this);
  //        }
...
#endif // TINYSWIFT_DIAGNOSTICENGINE_H
