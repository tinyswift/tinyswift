...
#ifndef TINYSWIFT_AST_PRINTOPTIONS_H
#define TINYSWIFT_AST_PRINTOPTIONS_H

#include "tinyswift/AST/AttrKind.h"
#include "tinyswift/AST/Identifier.h"
#include "tinyswift/AST/STLExtras.h"
...
typedef llvm::function_ref<void(llvm::ArrayRef<ExtensionAndIsSynthesized>)>
...
public:
  SynthesizedExtensionAnalyzer(NominalTypeDecl *Target, PrintOptions Options,
  ...
  bool shouldPrintRequirement(ExtensionDecl *ED, llvm::StringRef Req);
...
public:
  BracketOptions(Decl *Target = nullptr, bool OpenExtension = true,
      ...
      : Target(Target), OpenExtension(OpenExtension),
        CloseExtension(CloseExtension), CloseNominal(CloseNominal) {}
  ...
  bool shouldCloseNominal(const Decl *D) { return D != Target || CloseNominal; }
};
...
struct PrintOptions {
  ...
  /// Whether to print the content of an extension decl inside the type decl
  ...
  /// Retrieve the print options that are suitable to print the testable
  /// interface.
