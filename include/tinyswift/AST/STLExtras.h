...
inline void for_each3(const Container1 &c1, const Container2 &c2,
                      const Container3 &c3, TernaryFunction f) {
  assert(c1.size() == c2.size());
...
inline void set_union_for_each(InputIt1 I1, InputIt1 E1, InputIt2 I2,
  ...
  static_assert(
      std::is_same<typename std::iterator_traits<InputIt1>::value_type,
...
inline void set_union_for_each(const Container1 &C1, const Container2 &C2,
  ...
  static_assert(
      std::is_same<typename std::iterator_traits<
                       typename Container1::iterator>::value_type,
                   typename std::iterator_traits<
                       typename Container2::iterator>::value_type>::value,
...
template <typename Iterator>
inline IteratorRange<Iterator> makeIteratorRange(Iterator first,
                                                 Iterator last) {
...
template <typename Iterator, typename Predicate> class FilterIterator {
...
public:
  ...
  iterator end() const { return iterator(Last, Last, Pred, iterator::Primed); }
...
inline FilterRange<Range, Predicate> makeFilterRange(Range range,
                                                     Predicate pred) {
...
template <typename Iterator, typename Operation> class TransformIterator {
  ...
  /// operation.
  typedef
      typename std::iterator_traits<Iterator>::reference UnderlyingReference;

public:
  typedef std::forward_iterator_tag iterator_category;
  typedef
      typename std::result_of<Operation(UnderlyingReference)>::type value_type;
  ...
  reference operator*() const { return Op(*Current); }
...
public:
  ...
  TransformRange(Range range, Operation op) : Rng(range), Op(op) {}
...
inline TransformRange<Range, Operation> makeTransformRange(Range range,
                                                           Operation op) {
...
public:
  ...
  iterator begin() const { return iterator(First, Last, Op, iterator::Primed); }

  iterator end() const { return iterator(Last, Last, Op, iterator::Primed); }
...
template <typename Subclass, typename Range>
DowncastFilterRange<Subclass, Range> makeDowncastFilterRange(Range range) {
